...
$riserve = fetchAll($pdo, "SELECT r.*, p.nome as nomeProdotto, d.nome as nomeDispensa
                            FROM RISERVA r
    ...
    <?php foreach ($riserve as $r): ?>
        <div class="riserva-card">
            ...
            <div class="progress" style="margin-top: 1rem;">
                <div class="progress-bar" style="width: <?php echo min(100, ($r['quantitaAttuale'] / $r['quantitaIniziale']) * 100); ?>%;"></div>
            </div>
            <small class="text-muted">
                <?php echo formatWeight($r['quantitaAttuale'], 'kg'); ?> di <?php echo formatWeight($r['quantitaIniziale'], 'kg'); ?>
            </small>
        </div>
    <?php endforeach; ?>
</div>
