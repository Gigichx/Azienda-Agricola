...
if ($action === 'create') {
    ...
    if (!$idProdotto || !$idLuogo || empty($tipoLavorazione)
        || $quantitaIngresso <= 0 || $quantitaOttenuta <= 0) {
        redirectWithMessage('/admin/lavorazione.php', 'Dati non validi', 'error');
    }

    $sql = "INSERT INTO LAVORAZIONE
                (tipoLavorazione, dataLavorazione, quantitaIngresso, quantitaOttenuta, idProdotto, idLuogo)
            VALUES (?, ?, ?, ?, ?, ?)";
